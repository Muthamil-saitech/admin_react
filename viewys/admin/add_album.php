...
<div class="content-wrapper">
	...
	<div class="row">
		<div class="col-md-8 offset-md-2 grid-margin stretch-card">
			<div class="card">
				<div class="card-body">
					<h4 class="card-title"><?php if(isset($row)) { echo 'Edit'; } else  { echo 'Add'; } ?> Album</h4>
					<?php echo form_open_multipart(ADMIN.'/album/add', array('class' => 'forms-sample', 'data-parsley-validate' => 'true')); ?>
					<div class="form-group row">
						<label class="col-sm-2 col-form-label">Title <span class="text-danger">*</span></label>
						<div class="col-sm-10">
							<input type="hidden" name="album_id" readonly class="form-control" value="<?php if(isset($row['album_id'])) echo $row['album_id']; ?>" />
							<input type="text" name="album_title" id="album_title" class="form-control" value="<?php if(isset($posted['album_title'])) { echo $posted['album_title']; } else { if(isset($row['album_title'])) { echo $row['album_title']; } } ?>" maxlength="80" required />
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col-sm-6">
							<div class="btn btn-secondary">
								<input type="file" name="album_image" id="album_image" accept=".jpg,.png,.jpeg" <?php if(isset($row['album_image'])) { if($row['album_image'] == '') { echo 'required'; } } else { echo 'required'; } ?> />
							</div><br>
						...
						<div class="col-sm-4">
							<?php if(isset($row) && $row['album_image'] != '') { ?>
								<input type="hidden" name="old_image" readonly value="<?php echo $row['album_image'];  ?>" />									
								<img src="<?php  echo base_url('uploads/album/'.$row['album_image']);  ?>" class="img-fluid" alt="Image">
							<?php } ?>
						</div>
					</div>
