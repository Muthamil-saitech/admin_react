...
class Blogs_wo_fil extends Admin_Controller {
	public function __construct() {
        parent::__construct();
	}
	public function index(){
		$data['page_name'] = 'blogs_wo_fil';
		$this->view('blogs_wo_fil',$data);
	}
	public function add_blog($blog_id = false) {
		...
		$this->view('add_blog_wo_fil',$data);
	}
	public function add(){
		// print_a($_POST,1);
		...
		if($this->form_validation->run()==FALSE){
			...
			redirect(ADMIN.'/blogs_wo_fil/add_blog/'.$blog_id);
		} else {
			...
			if($check_unique_entry == 0) {
				...
				if(!empty($_FILES['image']['name'])) {
					...
					if(!$this->upload->do_upload('image')) {
						...
						redirect(ADMIN.'/blogs_wo_fil/add_blog/'.$blog_id,'refresh');
					}
					else {
				...
				redirect(ADMIN.'/blogs_wo_fil','refresh');
			} else  {
				$this->session->set_flashdata('posted',$_POST);
	...
	public function load_data(){
		...
		if($queryRecords) { foreach ($queryRecords as $k => $data) {
            ...
            $row[5] = '<a class="text-secondary edit-icon font-20" href="'.site_url(ADMIN.'/blogs_wo_fil/add_blog/').$data['blog_id'].'" title="Edit Blog"><i class="mdi mdi-pencil-outline"></i></a> &nbsp;&nbsp;<a class="text-secondary delete-icon font-20" style="cursor:pointer" onclick="delete_data('.$data['blog_id'].');" title="Delete Blog"><i class="mdi mdi-delete-outline"></i></a>';
            $tab_data[] = $row;
		} }
		$json_data = array(
