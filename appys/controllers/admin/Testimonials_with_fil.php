...
class Testimonials_with_fil extends Admin_Controller {
	public function __construct() {
        parent::__construct();
	}
	public function index() {
		$data['page_name'] = 'testimonials_with_fil';
		$this->view('testimonials_with_fil',$data);
	}
	public function add_testimonial($testimonial_id = false) {
		...
		$this->view('add_testimonial_with_fil',$data);
	}
	public function add(){
		/* print_a($_POST,1); */
		...
		if($this->form_validation->run() == FALSE){
			...
			redirect(ADMIN.'/testimonials_with_fil/add_testimonial/'.$testimonial_id);
		}else{
			...
			if(!empty($_FILES['image']['name'])) {
				...
				if(!$this->upload->do_upload('image')) {
					...
					redirect(ADMIN.'/testimonials_with_fil/add_testimonial/'. $testimonial_id,'refresh');
				} else {
			...
			redirect(ADMIN.'/testimonials_with_fil','refresh');
		}
	}
	public function load_data(){
		...
		if($queryRecords) { foreach ($queryRecords as $k => $data){
			...
			$row[5] = '<a class="text-secondary edit-icon font-20" href="'.site_url(ADMIN.'/testimonials_with_fil/add_testimonial/').$data['testimonial_id'].'" title="Edit Testimonial"><i class="mdi mdi-pencil-outline"></i></a> &nbsp;&nbsp;<a class="text-secondary delete-icon font-20" style="cursor:pointer" onclick="delete_data('.$data['testimonial_id'].');" title="Delete Testimonial"><i class="mdi mdi-delete-outline"></i></a>';
            $tab_data[] = $row;
		} }
		$json_data = array(
