...
class Banners_wo_fil extends Admin_Controller {
	public function __construct() {
        parent::__construct();
	}
	public function index() {
		$data['page_name'] = 'banners_wo_fil';
		$this->view('banners_wo_fil',$data);
	}
	public function add_banner($banner_id = false) {
		...
		$this->view('add_banner_wo_fil',$data);
	}
	public function add(){
		// print_a($_POST,1);
		...
		if($this->form_validation->run()==FALSE){
			...
			redirect(ADMIN.'/banners_wo_fil/add_banner/'.$banner_id);
		} else {
			...
			if(!empty($_FILES['image']['name'])) {
				...
				if(!$this->upload->do_upload('image')) {
					...
					redirect(ADMIN.'/banners_wo_fil/add_banner/'. $banner_id,'refresh');
				} else {
			...
			if($check_sequence_exists > 0) {
				...
				redirect(ADMIN.'/banners_wo_fil','refresh');
			}
		}
	}
